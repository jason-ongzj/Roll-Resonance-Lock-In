...
class RollResonance:
    ...
    #
    #     # Find solution to first order ODE for roll equation. Find the derivative of the roll rate for each time step,
    #     # then use Euler's method to find the roll rate for the next time step. Start time to be taken at the time when
    #     # the rocket has just left the rail.
    #     # A = q*Sref*d*self.clp(mach)*d*0.5/(self.V[i] * ixx)
    #     A = q*Sref*d*clp*d*0.5/(self.V[i]*ixx)
    #
    #     # B = q*Sref*d*(self.cldelta(mach)*cant)- self.CGoffset*self.cna(mach)*self.trimAoA[i]*sin(radians(self.roll[i] - self.gamma))))/ixx
    #     B = q*Sref*d*(cldelta*cant- self.CGoffset*cna*self.trimAoA[i]*sin(radians(self.roll[i] - self.gamma)))/ixx
    #
    #     C = (self.P[i] + B/A)/math.exp(A*(self.time[i] - self.time[start]))
    #     self.Pdot[i] = A*C*math.exp(A*(self.time[i]-self.time[start]))
    #     self.P[i+1] = self.P[i] + self.Pdot[i]*delta_t
    ...
    def CalcDynamicRoll(self, i, start):
        ...
        if(self.time[i+1] <= 20.1):
            self.P[i+1] = -B/A #+ (B/A)*math.exp(A*(self.time[i]-self.time[start]))
        else:
