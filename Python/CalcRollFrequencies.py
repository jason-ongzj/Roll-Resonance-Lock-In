...
# x1 = RollResonance.RollResonance("CN_A.txt", "CL_Delta.txt", "CL_P.txt", "CMQ.txt")
# x1.InitInputData("Dynamic_Roll.txt")
# x1.SetRefQuantities(0.205, 0.03301, 0.25)
# x1.FindNaturalFreq()
# x1.InitializeVars()
# x1.AssumeQuantities(0.00436, 22.5, Toffset, cg_offset, static_AoA, 0)
# x1.InitializeBC(100, -4.3138E-15, -0.003732135, 0, 2.144275746)
#
# for i in range(101, x1.time.size-1):
#     x1.CalcDynamicRoll(i, 101)

# ------------------------------------------------------------------------------------------------------------------
# Fin cant angle 0.75

# x2 = RollResonance.RollResonance("CN_A.txt", "CL_Delta.txt", "CL_P.txt", "CMQ.txt")
# x2.InitInputData("Dynamic_Roll.txt")
# x2.SetRefQuantities(0.205, 0.03301, 0.75)
# x2.FindNaturalFreq()
# x2.InitializeVars()
# x2.AssumeQuantities(0.00436, 22.5, Toffset, cg_offset, static_AoA, 0)
# x2.InitializeBC(100, -4.3138E-15, -0.003732135, 0, 2.144275746)
#
# for i in range(101, x2.time.size-1):
#     x2.CalcDynamicRoll(i, 101)
...
plt.clf()
# plt.plot(x.time, x1.P/360,label='0.25\N{DEGREE SIGN} Fin Cant')
plt.plot(x.time, x.P/360,label=str(fin_cant)+'\N{DEGREE SIGN} Fin Cant')
# plt.plot(x.time, x2.P/360,label='0.75\N{DEGREE SIGN} Fin Cant')
plt.plot(x.time, x.omega, label='Natural Frequency')
plt.ylabel('Frequency (Hz)', fontsize=12)
plt.xlabel('Time (s)', fontsize=12)
plt.axis([0,100,0,12])
...
ax.set_aspect(1)
# plt.plot(x.Sideslip[101:2100] + x.trimArmReal[101:2100], x.AoA[101:2100] + x.trimArmImag[101:2100], label="Tricyclic")
